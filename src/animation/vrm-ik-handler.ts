...
import { MathUtils, clampVector3ByRadian, VRM } from './math-utils';
import { Object3D, Bone } from './math-utils';


declare const VRMHumanBoneName: {
    readonly Hips: "hips";
...
type VRMHumanBoneName = (typeof VRMHumanBoneName)[keyof typeof VRMHumanBoneName];
...
export class VRMIKHandler {
  ...
  private constructor(public model: VRM) {
    ...
    this.iks.set(leftFootId, {
      ...
      links: [{
        ...
        rotationMin: new Vector3(-180, 0, 0).scale(
          MathUtils.DEG2RAD
        ),
  ...
  update() {
    for (const ik of this.iks.values()) {
      ...
      setFromMatrixPosition(targetPos, target.matrixWorld);
      ...
      for (let j = 0; j < iteration; j++) {
        ...
        for (const { enabled, index, rotationMin, rotationMax } of ik.links) {
          ...
          setFromMatrixPosition(effectorPos, effector.matrixWorld);
          effectorVec
            // .subVectors(effectorPos, linkPos)
            .subtract(effectorPos).subtract(linkPos)
            // .applyQuaternion(quaternion)
            .applyRotationQuaternion(quaternion)
            .normalize();
          targetVec
            // .subVectors(targetPos, linkPos)
            .subtract(targetPos).subtract(linkPos)
            // .applyQuaternion(quaternion)
            .applyRotationQuaternion(quaternion)
            .normalize();
          ...
          axis.cross(effectorVec).cross(targetVec).normalize();
          
          // link.quaternion.multiply(quaternion.setFromAxisAngle(axis, angle));
          link.quaternion.multiply(Quaternion.RotationAxis(axis, angle));
...
function setFromMatrixPosition(targetPos: any, matrixWorld: any) {
}
